...
- `row_pointers` — индексы начала строк
- `col_indices` — номера столбцов ненулевых элементов
- `values` — значения ненулевых элементов

Сам алгоритм:

1. Внешний цикл (`for i`): Перебирает все строки i матрицы A. Каждая строка i матрицы A будет определять i-ю строку в результирующей матрице C.

2. Цикл по ненулевым элементам i-й строки A (for k):

- Используя `A.row_pointers[i]` и `A.row_pointers[i+1]`, мы определяем диапазон ненулевых элементов в i-й строке A.

- Каждый ненулевой элемент Ai,k​ имеет значение `val_A` и столбцовый индекс `col_A` (который соответствует индексу строки k в матрице B).

3. Цикл по ненулевым элементам строки k матрицы B (`for l`):

- Столбцовый индекс `col_A` (из A) используется для поиска k-й строки в матрице B с помощью `B.row_pointers[col_A]`.

- Мы перебираем все ненулевые элементы Bk,j​ в k-й строке B. Каждый элемент имеет значение `val_B` и столбцовый индекс `col_B` (который соответствует индексу столбца j в матрице C).

4. Накопление: Произведение `val_A * val_B` вносится в соответствующий элемент Ci,j​ путем накопления в массиве `row_values`:\
`row_values[col_B] += val_A * val_B`
...
#### Измерения "чистого" времени вычислений (умножение матриц) - task_run
